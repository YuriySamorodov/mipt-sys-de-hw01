...
class UserActionLogger:
    ...
    def _get_last_session_action(self, user_id: int, session_id: str) -> Optional[dict]:
        """Возвращает последнее действие пользователя В ТЕКУЩЕЙ СЕССИИ из лога"""
        try:
            df = pd.read_csv(self.log_file)
            user_actions = df[(df['user_id'] == user_id) & (df['session_id'] == session_id)]
    ...
    def _get_last_user_action(self, user_id: int) -> Optional[dict]:
        """Возвращает последнее действие пользователя (любой сессии)"""
        try:
            df = pd.read_csv(self.log_file)
    ...
    def log_action(self, user_id: int, action: ActionType, is_session_end: bool = False):
        ...
        duration = 0.0
        last_action = self._get_last_session_action(user_id, session_id)  # Changed this line
